...
public class MainPage extends SharedPage {
	...
	//			if (getText(titleEl).contains(name)) {
	//				Actions a = new Actions(driver);
	//				waiting(1000);
	//				WebElement mouseBtn = el.findElement(By.cssSelector(".taskactionbtn"));
	//				waiting(1000);
	//				a.moveToElement(titleEl).build().perform();
	//				waiting(1000);
	//				click(mouseBtn);
	//				break;
	//			}
	...
	//	public void moveTask(int position, String name) {
	//		waiting(1000);
	//		for (WebElement el : tasksList) {
	//			WebElement titleEl = el.findElement(By.cssSelector(".task-title"));
	//			if (getText(titleEl).contains(name)) {
	//				Actions a = new Actions(driver);
	//				waiting(1000);
	//				// a.dragAndDrop(el, tasksList.get(1)).build().perform();
	//				a.clickAndHold(el).build().perform();
	//				a.moveToElement(tasksList.get(position)).build().perform();
	//				waiting(1000);
	...
	public void deleteRowTask(String name) {
		waiting(1000);
		for (WebElement el : distanceList) {
			WebElement taskTitle = el.findElement(By.cssSelector(".mtt-tabs.ui-sortable>li>a>span"));
			if (getText(taskTitle).contains(name) && getText(taskTitle) != "ofir task"
					&& getText(taskTitle) != "All tasks") {
				click(taskTitle);
				waiting(1000);
				WebElement optionBtn = el.findElement(By.cssSelector("li .list-action.mtt-img-button"));
				click(optionBtn);
				waiting(1000);
				click(deleteBtn);
				click(deleteOKBtn);
	...
	//		try {
	//			int number = Integer.parseInt(getText(numOfTask));
	//			System.out.println("the number of task is " + number);
